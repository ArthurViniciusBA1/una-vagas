    ...
    <div className="w-full">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {vagas.length === 0 ? (
          ...
          vagas.map((vaga: any) => (
            <div key={vaga.id} className="bg-card border border-border rounded-lg p-6 shadow-sm flex flex-col hover:shadow-md transition-shadow">
              <div className="flex items-center mb-4">
                {vaga.empresa?.logoUrl && (
                  <img src={vaga.empresa.logoUrl} alt={vaga.empresa.nome || 'Logo da Empresa'} className="w-12 h-12 rounded-full mr-4 object-cover" />
                )}
                <div>
              ...
              <div className="text-sm text-gray-600 mb-3">
                <p><strong>Tipo:</strong> {vaga.tipo.replace(/_/g, ' ').toLowerCase()}</p>
                <p><strong>Local:</strong> {vaga.localizacao}</p>
                {vaga.faixaSalarial && <p><strong>Salário:</strong> {vaga.faixaSalarial}</p>}
              </div>
