...
import { Award, Briefcase, EyeIcon, FileText, Languages, Lightbulb, Loader2, Star, UserCircle2 } from 'lucide-react';
    ...
    return (
      <div className='flex flex-col items-center justify-center h-screen text-destructive p-6'>
        ...
        <p className='mt-4 text-center'>
          Por favor, tente{' '}
          <Link href='/entrar' className='underline font-semibold'>
            fazer login
          </Link>{' '}
          novamente.
        </p>
  ...
  const secoesCurriculo = [
    {
      ...
      filled: !!curriculo?.titulo,
    },
    {
      nome: 'Experiências Profissionais',
      icon: Briefcase,
      action: () => setActiveModal('experiencia'),
      filled: (curriculo?.experienciasProfissionais?.length ?? 0) > 0,
    },
    {
      nome: 'Formação Acadêmica',
      icon: FileText,
      action: () => setActiveModal('formacao'),
      filled: (curriculo?.formacoesAcademicas?.length ?? 0) > 0,
    },
    {
      nome: 'Habilidades',
      icon: Star,
    ...
    },
    {
      ...
      filled: (curriculo?.projetos?.length ?? 0) > 0,
    },
    {
      nome: 'Certificações',
      icon: Award,
  ...
  return (
    <div className='w-full p-4 md:p-8'>
      <header className='mb-8 md:mb-10'>
        <h1 className='text-3xl md:text-4xl font-bold text-foreground'>Bem-vindo(a), {candidato.nome || 'Candidato'}!</h1>
      ...
      <section className='mb-10'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
          {secoesCurriculo.map((secao) => (
            ...
            >
              <secao.icon size={20} className='mr-3 text-primary group-hover:text-accent-foreground shrink-0' />
      ...
      <DashboardModalsManager activeModal={activeModal} setActiveModal={setActiveModal} />
    </div>
  );
}
