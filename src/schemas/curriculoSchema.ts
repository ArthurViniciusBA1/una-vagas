...
export const curriculoInformacoesPessoaisSchema = z.object({
  ...
  resumoProfissional: z.string().max(2000, "O resumo não deve exceder 2000 caracteres.").or(z.literal("")).optional(),
  telefone: z.string().max(20, "Telefone muito longo.").or(z.literal("")).optional(),
  endereco: z.string().max(255, "Endereço muito longo.").or(z.literal("")).optional(),
  linkedinUrl: z.string().url({ message: "URL do LinkedIn inválida." }).or(z.literal("")).optional(),
  githubUrl: z.string().url({ message: "URL do GitHub inválida." }).or(z.literal("")).optional(),
  portfolioUrl: z.string().url({ message: "URL do Portfólio inválida." }).or(z.literal("")).optional(),
});
...
export const experienciaProfissionalSchema = z.object({
  ...
  dataInicio: z.string({ required_error: "Data de início é obrigatória." }).min(7, "Formato inválido. Use AAAA-MM."),
  dataFim: z.string().or(z.literal("")).optional(),
  trabalhoAtual: z.boolean(),
  descricao: z.string().max(2500, "A descrição é muito longa.").or(z.literal("")).optional(),
});
...
export const projetoSchema = z.object({
  ...
  descricao: z.string().max(2000, "Descrição muito longa.").or(z.literal("")).optional(),
  projectUrl: z.string().url({ message: "URL do projeto inválida." }).or(z.literal("")).optional(),
  repositorioUrl: z.string().url({ message: "URL do repositório inválida." }).or(z.literal("")).optional(),
});
...
export const certificacaoSchema = z.object({
  ...
  credencialUrl: z.string().url({ message: "URL da credencial inválida." }).or(z.literal("")).optional(),
});
